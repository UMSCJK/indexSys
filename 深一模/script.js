...
function getSel(obj) {
	...
	if (oSel.bio == true && oSel.pol == false && oSel.geo == false) {
		return "bio"
	} else if (oSel.bio == false && oSel.pol == true && oSel.geo == false) {
		return "pol"
	} else if (oSel.bio == false && oSel.pol == false && oSel.geo == true) {
		return "geo"
	} else {
		alert('发生错误！')
	}
...
function getSelTxt(obj, ot) {
	if (getSel(obj) == 'bio') {
		if (ot == 'ori') { return '生原' }
		else if (ot == 'trs') { return '生物' }
	}
	else if (getSel(obj) == 'pol') {
		if (ot == 'ori') { return '政原' }
		else if (ot == 'trs') { return '政治' }
	}
	else if (getSel(obj) == 'geo') {
		if (ot == 'ori') { return '地原' }
		else if (ot == 'trs') { return '地理' }
	}
}
function getLan(obj, txt) {
	if (data.main[obj].sel.jap == true) {
		if (txt == true) {
			return '日语'
		}else if (txt == false) {
			return 'jap'
		}
	} else if (data.main[obj].sel.jap == false) {
		if (txt == true) {
			return '英语'
		}else if (txt == false) {
			return 'eng'
